...
2. **時間帯指定**
   ...
   - WHEN ユーザーが時間帯を未選択のままにした THEN システムは全時間帯（8:30-21:00）を検索対象とする SHALL
   - WHEN ユーザーが特定の時間帯を選択した THEN システムはその時間帯のみを検索対象とする SHALL
...
4. **時間帯の展開/折りたたみ**
   ...
   - WHEN ユーザーが展開操作を行う THEN システムは全時間帯（空き/予約済み）を表示する SHALL
...
3. **テスト（将来拡張）**
   - 初期バージョンでは自動テストは実装しない
   - 将来的にユニットテスト、E2Eテストの追加を検討
