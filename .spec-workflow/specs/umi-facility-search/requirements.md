...
2. **時間帯指定**
   - WHEN ユーザーが検索フォームを表示した THEN システムは時間帯選択UIを表示する SHALL
   - システムは開始時刻（From）と終了時刻（To）を選択できるUIを提供する SHALL
   - WHEN ユーザーが時間帯を未選択のままにした THEN システムは全時間帯（8:30-21:30）を検索対象とする SHALL
   - WHEN ユーザーが開始時刻と終了時刻を選択した THEN システムはその時間範囲のみを検索対象とする SHALL

3. **柔軟な検索パターン**
   - システムは以下の検索パターンをサポートする SHALL:
     - 複数日 + 時間帯指定なし（全時間帯検索）
     - 複数日 + 時間範囲指定（From-To）
     - 単一日 + 時間帯指定なし
     - 単一日 + 時間範囲指定（From-To）
...
4. **タイムアウト処理**
   - IF スクレイピング処理が10秒を超える THEN システムはタイムアウトエラーを返す SHALL
...
1. **リセットボタン** ✅
   ...
   - WHEN ユーザーがリセットボタンをクリックした THEN システムはすべての入力内容（日付、時間範囲）をクリアする SHALL
   - リセットボタンは検索ボタンと並んで配置され、モバイルでは縦並び、PCでは横並びで表示される SHALL
   - リセットボタンは44px × 44px以上のタップ可能なサイズである SHALL

2. **エラー時の入力状態保持** ✅
   - WHEN 検索がエラーになった THEN システムは選択した日付と時間範囲を保持する SHALL
...
1. **可用性**
   - Vercel無料枠の範囲内で99%アップタイムを目標
   - ダウンタイムはVercelのメンテナンス時のみ
